...
class SchedulerBU extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      onEventMoved: args => {
        ...
        if (
          ((new Date(args.e.data.needed_date) - new Date(args.e.data.end.value)) / 60000) < 0
        ) {
          this.props.preventSave(args.e.data.needed_date_display_str);
          ...
          this.props.test(
            ...
            args.e.data.start.value,
            parseInt(args.e.data.int_duration_mins_team + args.e.data.ext_duration_mins_team)
          );
          ...
          this.props.test(
            ...
            args.e.data.start.value,
            args.e.data.start.value,
            parseInt(args.e.data.int_duration_mins_solo + args.e.data.ext_duration_mins_solo)
          );
        }
      },
                    ...
                    {this.props.shiftType === "team" && this.state.unscheduledOrders.length > 0 &&
                      <div>
                        {this.state.unscheduledOrders.map(wo => (
                          <div key={wo.id} className="work-order">
                            <input onChange={this.handleChange} type="radio" value={wo.id} name="selectedWorkOrder" id={wo.int_duration_mins_team + wo.ext_duration_mins_team}/>
                            <span className="work-order-title-section">
                              Trailer {wo.trailer_id}
                              <br/>
                              <span className="work-order-duration">
                                <i class="far fa-clock"></i> {
                                  Math.floor((wo.int_duration_mins_team + wo.ext_duration_mins_team) / 60)
                                  } hrs&nbsp;
                                  {
                                  ((wo.int_duration_mins_team + wo.ext_duration_mins_team) % 60) > 0 && <span>{(wo.int_duration_mins_team + wo.ext_duration_mins_team) % 60} mins</span>
                                  }
                              </span>
                            </span>
                    ...
                    {this.props.shiftType === "team" && this.state.unscheduledOrdersDisabled.length > 0 &&
                      <div>
                        {this.state.unscheduledOrdersDisabled.map(wo => (
                          <div onClick={this.handleAlert} key={wo.id} className="work-order">
                            <input onChange={this.handleChange} disabled={true} type="radio" value={wo.id} name="selectedWorkOrder" id={wo.int_duration_mins_team + wo.ext_duration_mins_team}/>
                            <span className="work-order-title-section">
                              Trailer {wo.trailer_id}
                              <br/>
                              <span className="work-order-duration">
